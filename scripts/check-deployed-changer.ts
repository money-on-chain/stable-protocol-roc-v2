...
import { MocCARC20__factory, FlowChangeProposal__factory, CommissionSplitter__factory } from "../typechain";
...
async function main() {
  ...
  if (contractInfo.mocCore.mocFeeFlowAddress.toLowerCase() !== feesSplitter.address.toLowerCase()) {
  ...
  if (contractInfo.mocCore.tcInterestCollectorAddress.toLowerCase() !== tcInterestSplitter.address.toLowerCase()) {
  ...
  if (contractInfo.mocCore.bes.toString() !== contractInfo.FlowChangeProposal.settlementBlockSpan.toString()) {
    console.log("--> ERROR!: No the same as the changer!");
  } else {
    console.log("--> OK");
  }
  ...
  if (contractInfo.mocCore.decayBlockSpan.toString() !== contractInfo.FlowChangeProposal.decayBlockSpan.toString()) {
    console.log("--> ERROR!: No the same as the changer!");
  } else {
    console.log("--> OK");
  }
